...
class case():
	...
	def _value(self, _globals, _locals):
		if self.iscode and isinstance(self.value, (str, bytes, bytearray)):
			exec(compile('def __get_switch_result__():' + self.value.replace('\n', '\n\t'), [], 'exec'), _globals, _locals)
			return __get_switch_result__()
		# if self.iscode and isinstance(self.value, (str, bytes, bytearray)):
...
def foobar():
	print("im in foobar")
sw = switch(varname='varname1')[
	case(7, code=1):
		'_ = 1; print("hey, look at: \'{}\'!".format(varname1))' : br,
	case(3, code=1):
		'return 9' : br,
	case(4, code=1):
"""
